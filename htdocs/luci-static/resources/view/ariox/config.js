	...
	if (path === 'ariox') {
		L.resolveDefault(L.uci.load('aria2'), null).then(function() {
			...
			if (authMethod === 'token') {
				...
				var user = L.uci.get('aria2', 'main', 'rpc_user') || '';
				var pass = L.uci.get('aria2', 'main', 'rpc_passwd') || '';
				url += '/?user=' + encodeURIComponent(user) + '&pass=' + encodeURIComponent(pass) + '&port=' + port;
			} else {
				url += '/?port=' + port;
			}
			window.open(url);
...
return view.extend({
	...
	render: function(aria2) {
		...
		s = m.section(form.NamedSection, 'main', 'aria2', _('Settings'));
		s.addremove = false;
		s.anonymous = true;
		...
		o = s.taboption('basic', form.DummyValue, '_hr1');
		...
		if (aria2.bt) {
			...
			o = s.taboption('bt', form.Value, 'bt_tracker', _('BT tracker'),
			_('A list of BitTorrent trackers.'));
		o.placeholder = 'http://192.168.1.1:2710/announce,udp://192.168.1.1:2710/announce';
	}
